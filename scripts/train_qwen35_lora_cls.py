#!/usr/bin/env python3
"""
Fine-tune Qwen3.5-2B or Qwen3.5-4B with LoRA (r=16) + linear classification head on CheXpert.

Requires: pip install peft transformers>=4.52.0
"""
...
def main():
    parser = build_standard_argparser("Train Qwen3.5 LoRA r=16 + classification head.")
    ...
    backbone = load_base_qwen35_model(
        model_dir,
        device,
    ...
    model = Qwen35Classifier(backbone, hidden_size, num_labels).to(device)
    ...
    run_id = args.run_id or auto_run_id(f"qwen35_{variant}_lora_r16")
    ...
    base_reload = load_base_qwen35_model(
        model_dir,
        device,
    ...
    metrics = {
        "variant": f"qwen35_{variant}_lora_r16_cls",
    ...
    write_json(out_dir / "run_meta.json", {"mode": "cls", "adapter_dir": str(adapter_dir), "variant": variant})
    ...
    print({"run_dir": str(out_dir), "test_macro_f1@0.5": test_f1, "trainable_params": trainable, "variant": variant})
