#!/usr/bin/env python3
"""
Fine-tune Qwen3.5-2B or Qwen3.5-4B with LoRA (r=16) via generative JSON SFT (CheXpert labels).
...
def main():
    parser = build_standard_argparser("Train Qwen3.5 LoRA r=16 with generative JSON SFT.")
    ...
    model_id = args.model_id or default_lora_sft_model_id(variant)
    ...
    run_id = args.run_id or auto_run_id(f"qwen35_{variant}_lora_r16_sft")
    out_dir = resolve_experiment_dir(
        out_dir=args.out_dir or None,
        model_id=model_id,
        protocol=args.protocol or "default",
        run_id=run_id,
        default_legacy_out_dir=f"data/processed/experiments/qwen35_{variant}_lora_r16_sft",
    )
    ...
    base_reload = load_base_qwen35_model(model_dir, device, gradient_checkpointing=False)
    ...
    metrics = {
        "variant": f"qwen35_{variant}_lora_r16_sft",
    ...
    write_json(
        out_dir / "run_meta.json",
        {
            ...
            "parse_failures_test": test_parse_fail,
            "variant": variant,
        },
    )
